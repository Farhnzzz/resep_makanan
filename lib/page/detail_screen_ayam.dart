...
class DetailScreenAyam extends StatefulWidget {
  final ModelAyam ayam;

  const DetailScreenAyam({Key? key, required this.ayam}) : super(key: key);

  @override
  _DetailScreenAyamState createState() => _DetailScreenAyamState();
}

class _DetailScreenAyamState extends State<DetailScreenAyam> {
  ...
  Future<void> simpanResep() async {
    ...
    String resepJson = jsonEncode(widget.ayam.toJson());
    ...
    print(
        'Resep ${isResepDisimpan ? "disimpan" : "dihapus"}: ${widget.ayam.nama}');
  }
  ...
  Future<void> hapusResep() async {
    ...
    // Tambahkan pesan log atau penanganan lain yang diperlukan
    print('Resep dihapus dari penyimpanan: ${widget.ayam.nama}');
  }
...
return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            // Tampilkan gambar resep dengan border radius circular
            Container(
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(20.0),
                image: DecorationImage(
                  image: NetworkImage(widget.ayam.gambar?.toString() ?? ''),
                  fit: BoxFit.cover,
                ),
              ),
              height: 200,
            ...
            Row(
              ...
              children: [
                // Waktu Masak
                buildInfoItem(Icons.access_time, widget.ayam.waktuMasak ?? ''),
                // Porsi
                buildInfoItem(Icons.restaurant, widget.ayam.porsi ?? ''),
                // Tingkat Kesulitan
                buildInfoItem(Icons.star, widget.ayam.tingkatKesulitan ?? ''),
              ],
            ),
            SizedBox(height: 16.0),
