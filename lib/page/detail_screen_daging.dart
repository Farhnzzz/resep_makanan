...
class DetailScreenDaging extends StatefulWidget {
  final ModelDaging daging;

  const DetailScreenDaging({Key? key, required this.daging}) : super(key: key);

  @override
  _DetailScreenDagingState createState() => _DetailScreenDagingState();
}

class _DetailScreenDagingState extends State<DetailScreenDaging> {
  ...
  Future<void> simpanResep() async {
    ...
    String resepJson = jsonEncode(widget.daging.toJson());
    ...
    print(
        'Resep ${isResepDisimpan ? "disimpan" : "dihapus"}: ${widget.daging.nama}');
  }
  ...
  Future<void> hapusResep() async {
    ...
    // Tambahkan pesan log atau penanganan lain yang diperlukan
    print('Resep dihapus dari penyimpanan: ${widget.daging.nama}');
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: Icon(isResepDisimpan ? Icons.bookmark : Icons.bookmark_border),
            onPressed: () async {
              if (isResepDisimpan) {
                await hapusResep();
          ...
          children: [
            ...
            Container(
              decoration: BoxDecoration(
                ...
                image: DecorationImage(
                  image: NetworkImage(widget.daging.gambar.toString()),
                  fit: BoxFit.cover,
                ),
              ),
              height: 200,
            ...
            Row(
              ...
              children: [
                // Waktu Masak
                buildInfoItem(Icons.access_time, widget.daging.waktuMasak!),
                // Porsi
                buildInfoItem(Icons.restaurant, widget.daging.porsi!),
                // Tingkat Kesulitan
                buildInfoItem(Icons.star, widget.daging.tingkatKesulitan!),
              ],
            ),
            SizedBox(height: 16.0),
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: widget.daging.bahan!.map((bahan) => buildIngredientCard(bahan)).toList(),
            ),
            SizedBox(height: 16.0),
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: widget.daging.langkah!.asMap().map((index, langkah) =>
